...
class NoticesState extends State<Notices> {
  ...
  Container noNoticesFoundYet(){
    return Container(
      child: Column(
        children: [
          Image.asset("Resources/Icons/none.png", width: 80,),
          SizedBox(height: 10),
          Text(Strings.shared.controllers.notices.noPushNotificationYet, style: TextStyle(fontSize: Statics.shared.fontSizes.titleInContent,color: Statics.shared.colors.subTitleTextColor),)
        ],
            ...
            widget.db.selectAnswer(idx: results[i].idx,onResult: (items){
              if(items.length > 0){
                ...
                if(answer["voteDone"] == "FALSE"){
                  ...
                  showPopUp(
                      ...
                      onPressApply: (){
                        ...
                        Navigator.pop(_scaffold.currentContext);
                        showPopUpVotingComplete(onPressOk:(){Navigator.pop(_scaffold.currentContext);},noticeType: NoticeType.Vote);
                      }
                  );
                }else{
                ...
                MainTabBar.myChild.getUserId(onGetUserId: (uid){
                  String url = "${results[i].httpPath.toString()}&userId=${uid}&mode=view";
                  url = url.replaceAll("https://", "http://");
                  MainTabBar.myChild.getVoteAnswers(url: url,onSent: (response){

                    widget.db.insertAnswer(onAdded: (){
                      showSnackBar("Answers have been catched successfully.");
                      if(Notices.staticNoticesPage != null){
                        refreshNotices();
                      }
                    },idx: results[i].idx.toString(), status: response['data']['status'].toString(),
