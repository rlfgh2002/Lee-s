...
class _SurveysTabsState extends State<SurveysTabs> with TickerProviderStateMixin {
  ...
  showPopUp({String content,String idx,String votingPeriod,List<String> votes, VoidCallback onPressApply, VoidCallback onPressClose, NoticeType nt = NoticeType.Vote, List<Map<String, dynamic>> surveys}){
    ...
    showDialog(context: context,builder: (BuildContext context) {
      if(nt == NoticeType.Vote){
        return HaegisaAlertDialog(votes: votes,
  ...
  showPopUpVotingComplete({VoidCallback onPressOk, NoticeType noticeType = NoticeType.Vote}){
    ...
    showDialog(context: context,builder: (BuildContext context) {
      if(noticeType == NoticeType.Vote){
        return HaegisaAlertCompleteDialog(popUpWidth: popUpWidth,
  ...
  showSnackBar(String text){
    ...
    Scaffold.of(context).showSnackBar(SnackBar(
      content: Text(text, style: TextStyle(color: Colors.white, fontSize: Statics.shared.fontSizes.content),),
      backgroundColor: Colors.black,
      duration: Duration(seconds: 3),
    ));
  }
  Container showTopBarTitle(){
    return Container(child: Text(Strings.shared.controllers.notices.pageTitle, style: TextStyle(color: Statics.shared.colors.titleTextColor, fontSize: Statics.shared.fontSizes.title)),margin: const EdgeInsets.only(left: 8));
  }
  ...
  void refreshNotices(){
    ...
    widget.db.selectSurvey(onResult: (res){
      ...
      for(int i = 0; i < res.length; i++){
        NoticeWidget item = NoticeWidget(isOnSurveysTabs: true,title: res[i]['subject'].toString(),shortDescription: res[i]['contents'].toString(),time: res[i]['start_date'].toString(),type: NoticeType.Survey,
          onTapped: (){
              ...
              if(surveys.length > 0){
                ...
                showPopUp(
                    ...
                    onPressApply: (){
                      ...
                      Navigator.pop(_scaffold.currentContext);
                      showPopUpVotingComplete(onPressOk:(){Navigator.pop(_scaffold.currentContext);},noticeType: NoticeType.Survey);
                    }
                );
              }else{
                showPopUp(
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.selectedTab == 0){
      tab1 = Tab(child: Text("진행중인 설문", style: TextStyle(color: selectedTabColor, fontSize: Statics.shared.fontSizes.content)));
      tab2 = Tab(child: Text("종료된 설문", style: TextStyle(color: normalTabColor, fontSize: Statics.shared.fontSizes.content)));
    }else{
      tab1 = Tab(child: Text("진행중인 설문", style: TextStyle(color: normalTabColor, fontSize: Statics.shared.fontSizes.content)));
      tab2 = Tab(child: Text("종료된 설문", style: TextStyle(color: selectedTabColor, fontSize: Statics.shared.fontSizes.content)));
    }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Container(child: Text(Strings.shared.controllers.SurveysTabs.pageTitle, style: TextStyle(color: Statics.shared.colors.titleTextColor, fontSize: Statics.shared.fontSizes.titleInContent, fontWeight: FontWeight.w100)),margin: const EdgeInsets.only(left: 0)),
        ),
        body: TabBarView(
          children: [
            widget.List1,
