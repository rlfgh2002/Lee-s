...
class HaegisaAlertSurveyDialog extends StatefulWidget {
  ...
  HaegisaAlertSurveyDialog({String endDate, String startDate,bool isDone,String idx,double popUpWidth = 0,double popUpHeight = 0,String content,String votingPeriod,List<Map<String, dynamic>> surveys, VoidCallback onPressClose, VoidCallback onPressApply, VoidCallback afterSubmit}){
        ...
        if(body['code'].toString() == "204"){
          ...
          for(int i = 0; i < qCnt; i++){
            for(int j = 0; j < 4; j++){
              ...
              print("=====>>>> ${strPercent.toString()}  - q${i+1}_${j+1}_result_percent");

              int newNum = j+1;
              int newNum2 = i+1;
              this.widget.db.updateSurveyResult(idx: body['table'][0]['bd_idx'].toString(), resNum: "${newNum.toString()}",qNumber: "${newNum2.toString()}",result: strPercent,onUpdated: (){
                print(".....::::::::: SURVAY UPDAING (result${j+1})={${strPercent.toString()}} RESULT IN DATABASE :::::::::.....");
              });

              this.widget.db.updateSurveyisDone(idx: body['table'][0]['bd_idx'].toString(), surveyDone: "TRUE", onUpdated: (){
                print(".....::::::::: SURVAY UPDAING DONE IN DATABASE :::::::::.....");
              });
          ...
          this.widget.db.selectSurveyAnswer(idx: this.widget.idx,onResult: (res){
    ...
    for(int i = 0; i < widget.surveys.length; i++){
      ...
      for(int j = 0; j < 8; j++){
        if(this.widget.surveys[i]['q${j+1}'] != ""){
          ...
          mySurveysList.add(SurveyWidget(myIndex: j,surveyObj: this.widget.surveys[i],qNum: this.widget.surveys[i]['qNumber'].toString(),surveyIdx: this.widget.surveys[i]['surveyIdx'].toString(), width: this.widget.popUpWidth - 64,survey: this.widget.surveys[i]['q${j+1}'],groupName: "voteGroup",itemIndex: i,isChecked: true,isAfter: true));
        }
      }//loop2
    }
    return Container(
