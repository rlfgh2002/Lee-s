    ...
    MainTabBar.myChild.getUserId(onGetUserId: (uid) {
      http.post(Statics.shared.urls.submitSurvey(), body: {
        ...
        if (body['code'].toString() == "204") {
                          ...
                          if (item['result5'] != null) {
                            MainTabBar.mainTabBar.mdw.lastSurveyPercentages.add(
                                SurveyResultPercentageObject(
                                    qn: i + 1,
                                    res:
                                        int.parse(item['result5'].toString())));
                          }
                          if (item['result6'] != null) {
                            MainTabBar.mainTabBar.mdw.lastSurveyPercentages.add(
                                SurveyResultPercentageObject(
                                    qn: i + 1,
                                    res:
                                        int.parse(item['result6'].toString())));
                          }
                          if (item['result7'] != null) {
                            MainTabBar.mainTabBar.mdw.lastSurveyPercentages.add(
                                SurveyResultPercentageObject(
                                    qn: i + 1,
                                    res:
                                        int.parse(item['result7'].toString())));
                          }
                          if (item['result8'] != null) {
                            MainTabBar.mainTabBar.mdw.lastSurveyPercentages.add(
                                SurveyResultPercentageObject(
                                    qn: i + 1,
                                    res:
                                        int.parse(item['result8'].toString())));
                          }
                        });
                      });
                } // for loop
          ...
          int qCount =
              int.parse(body['table'][0]['q_title']['q1_total'].toString());
          for (int i = 0; i < qCnt; i++) {
            for (int j = 0; j < qCount; j++) {
              ...
              this.widget.db.updateSurveyResult(
                  ...
                  onUpdated: () {
                    print(
                        ".....::::::::: SURVAY UPDAING (q${j + 1})={${strPercent.toString()}%} RESULT IN DATABASE :::::::::.....");
                  });
