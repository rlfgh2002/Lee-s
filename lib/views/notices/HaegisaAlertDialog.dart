...
class HaegisaAlertDialog extends StatefulWidget {
  ...
  HaegisaAlertDialog({String idx,double popUpWidth = 0,double popUpHeight = 0,String content,String votingPeriod,List<String> votes, VoidCallback onPressClose, VoidCallback onPressApply}){
...
class _HaegisaAlertDialogState extends State<HaegisaAlertDialog> {
  ...
  void submitVote({onSent(Map<String, dynamic> jj), onNotSent(), String idx, String check = "a1"}) async {
  ...
  Widget build(BuildContext context) {
    ...
    for(int i = 0; i < widget.votes.length; i++){
      myVoteList.add(
        ...
        VoteWidget(width: this.widget.popUpWidth - 64,vote: this.widget.votes[i],groupName: "voteGroup",itemIndex: i, onTapped: (){
          setState(() {
      ...
      content: Container(
        child: Column(
          children: [
            ...
            Container(padding: const EdgeInsets.only(left: 32, right: 32),
              child: Row(
                children: [
                  ...
                  SizedBox(width: 5),
                  Text(Strings.shared.dialogs.votingPeriod,style: TextStyle(color: Statics.shared.colors.mainColor, fontSize: Statics.shared.fontSizes.supplementary, fontWeight: FontWeight.bold),),
                  SizedBox(width: 5),
            ...
            SizedBox(height: 30),
            Container(
              child: Container(child: ListView(children: myVoteList), height: 220,padding: const EdgeInsets.only(left: 16, right: 16),),
            ), // vote List
            SizedBox(height: 30),
            Container(child: Text(Strings.shared.dialogs.bottomLableVoteDialog,style: TextStyle(color: Statics.shared.colors.subTitleTextColor, fontSize: Statics.shared.fontSizes.supplementary),),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Container(color: Statics.shared.colors.mainColor, child: FlatButton(child: Text(Strings.shared.dialogs.applyVoteBtn, style: TextStyle(fontWeight: FontWeight.bold,fontSize: Statics.shared.fontSizes.subTitleInContent, color: Colors.white),),onPressed: (){

                    // Send Vote Result To Server From Here ...
                    var trueInt = 0;
                    for(int i = 0; i < widget.voteGroupItems.length; i++){
                      if(widget.voteGroupItems[i]){
                        trueInt = i;
                      }
                    }
                    submitVote(idx: widget.idx,check: "a${(trueInt + 1).toString()}",onSent: (res){
                      if(res['code'].toString() == "0"){
                        print("You have Already Voted.");
                        // you have already voted.
                      }else{
                        print("Voting was Successful.");
                        widget.db.updateAnswer(idx: widget.idx,voteDone: "TRUE",onUpdated: (){
