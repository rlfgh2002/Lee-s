...
class ChatWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (this.time.contains("/")) {
      ...
      clearDateTime =
          "${dateSpt[0].toString()}-${dateSpt[1].toString()}-${dateSpt[2].toString()} at ${timeSpt[0].toString()}:${timeSpt[1].toString()}";
    } else {
      dt = DateTime.parse(this.time);
      clearDateTime =
          "${dt.year.toString()}-${dt.month.toString()}-${dt.day.toString()} at ${dt.hour.toString()}:${dt.minute.toString()}";
    }
